...
#region Math operators
		...
		private static void Sum(Evaluator eval, StackFrame frame)
		{
			...
			for (Atom iter = frame.args?.next; iter != null; iter = iter.next)
				res = AtomNumberOperations.Sum(res, ((Atom) iter.value).value);
		...
		private static void Dis(Evaluator eval, StackFrame frame)
		{
			...
			object res = ((Atom) frame.args?.value)?.value;
			for (Atom iter = frame.args?.next; iter != null; iter = iter.next)
				res = AtomNumberOperations.Dis(res, ((Atom) iter.value).value);
			eval.Return(new Atom(AtomType.Number, res));
		}
		...
		private static void Mul(Evaluator eval, StackFrame frame)
		{
			...
			object res = ((Atom) frame.args?.value)?.value;
			for (Atom iter = frame.args?.next; iter != null; iter = iter.next)
				res = AtomNumberOperations.Mul(res, ((Atom) iter.value).value);
			eval.Return(new Atom(AtomType.Number, res));
		}
		...
		private static void Div(Evaluator eval, StackFrame frame)
		{
			...
			object res = ((Atom) frame.args?.value)?.value;
			for (Atom iter = frame.args?.next; iter != null; iter = iter.next)
				res = AtomNumberOperations.Div(res, ((Atom) iter.value).value);
			eval.Return(new Atom(AtomType.Number, res));
		}
		...
		private static void Mod(Evaluator eval, StackFrame frame)
		{
			...
			object res = ((Atom) frame.args?.value)?.value;
			for (Atom iter = frame.args?.next; iter != null; iter = iter.next)
				res = AtomNumberOperations.Mod(res, ((Atom) iter.value).value);
			eval.Return(new Atom(AtomType.Number, res));
		}
...
#region Math functions
		...
		private static void Min(Evaluator eval, StackFrame frame)
		{
			...
			object res = ((Atom) frame.args?.value)?.value;
			for (Atom iter = frame.args?.next; iter != null; iter = iter.next)
				res = AtomNumberOperations.Min(res, ((Atom) iter.value).value);
			eval.Return(new Atom(AtomType.Number, res));
		}
		...
		private static void Max(Evaluator eval, StackFrame frame)
		{
			...
			object res = ((Atom) frame.args?.value)?.value;
			for (Atom iter = frame.args?.next; iter != null; iter = iter.next)
				res = AtomNumberOperations.Max(res, ((Atom) iter.value).value);
			eval.Return(new Atom(AtomType.Number, res));
		}
		...
		private static void Abs(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Abs(atom.value)));
		}
		...
		private static void Sign(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Sign(atom.value)));
		}
		...
		private static void Ceil(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Ceil(atom.value)));
		}
		...
		private static void Floor(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Floor(atom.value)));
		}
		...
		private static void Trunc(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Trunc(atom.value)));
		}
		...
		private static void Sqrt(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Sqrt(atom.value)));
		}
		...
		private static void Pow(Evaluator eval, StackFrame frame)
		{
			...
			object res = ((Atom) frame.args?.value)?.value;
			for (Atom iter = frame.args?.next; iter != null; iter = iter.next)
				res = AtomNumberOperations.Pow(res, ((Atom) iter.value).value);
			eval.Return(new Atom(AtomType.Number, res));
		}
		...
		private static void Exp(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Exp(atom.value)));
		}
		...
		private static void Logn(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Logn(atom.value)));
		}
		...
		private static void Log(Evaluator eval, StackFrame frame)
		{
			...
			object res = ((Atom) frame.args?.value)?.value;
			for (Atom iter = frame.args?.next; iter != null; iter = iter.next)
				res = AtomNumberOperations.Log(res, ((Atom) iter.value).value);
			eval.Return(new Atom(AtomType.Number, res));
		}
		...
		private static void Log10(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Log10(atom.value)));
		}
		...
		private static void Sin(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Sin(atom.value)));
		}
		...
		private static void Cos(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Cos(atom.value)));
		}
		...
		private static void Tan(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Tan(atom.value)));
		}
		...
		private static void Asin(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Asin(atom.value)));
		}
		...
		private static void Acos(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Acos(atom.value)));
		}
		...
		private static void Atan(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Atan(atom.value)));
		}
		...
		private static void Atan2(Evaluator eval, StackFrame frame)
		{
			...
			object res = ((Atom) frame.args?.value)?.value;
			for (Atom iter = frame.args?.next; iter != null; iter = iter.next)
				res = AtomNumberOperations.Atan2(res, ((Atom) iter.value).value);
			eval.Return(new Atom(AtomType.Number, res));
		}
		...
		private static void Sinh(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Sinh(atom.value)));
		}
		...
		private static void Cosh(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Cosh(atom.value)));
		}
		...
		private static void Tanh(Evaluator eval, StackFrame frame)
		{
			...
			eval.Return(new Atom(AtomType.Number, AtomNumberOperations.Tanh(atom.value)));
		}
...
#region Binary operators
		...
		{
			CheckAllNumbers(frame.args);
			object result = BitOperation(frame.args, AtomNumberOperations.And);
		...
		{
			CheckAllNumbers(frame.args);
			object result = BitOperation(frame.args, AtomNumberOperations.Or);
			eval.Return(new Atom(AtomType.Number, result));
		}
		...
		{
			CheckAllNumbers(frame.args);
			object result = BitOperation(frame.args, AtomNumberOperations.Xor);
			eval.Return(new Atom(AtomType.Number, result));
		}
		...
		{
			CheckAllNumbers(frame.args);
			object result = BitOperation(frame.args, AtomNumberOperations.Lsh);
			eval.Return(new Atom(AtomType.Number, result));
		}
		...
		{
			CheckAllNumbers(frame.args);
			object result = BitOperation(frame.args, AtomNumberOperations.Rsh);
			eval.Return(new Atom(AtomType.Number, result));
		}
...
#region Comparsion
		...
		{
			CheckAllNumbers(frame.args);
			bool result = CheckLogic(frame.args, AtomNumberOperations.Lt);
		...
		{
			CheckAllNumbers(frame.args);
			bool result = CheckLogic(frame.args, AtomNumberOperations.Gt);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			CheckAllNumbers(frame.args);
			bool result = CheckLogic(frame.args, AtomNumberOperations.Le);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			CheckAllNumbers(frame.args);
			bool result = CheckLogic(frame.args, AtomNumberOperations.Ge);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			CheckAllNumbers(frame.args);
			bool result = CheckLogic(frame.args, AtomNumberOperations.Ne);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			CheckAllNumbers(frame.args);
			bool result = CheckLogic(frame.args, AtomNumberOperations.Eq);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
...
#region Types Predicates
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.UBYTE);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.SBYTE);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.CHAR);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.USHORT);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.SSHORT);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.UINT);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.SINT);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.ULONG);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.SLONG);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.FLOAT);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
		...
		{
			Atom atom   = (Atom) frame.args?.value;
			bool result = CheckNumberType(atom, AtomNumberOperations.DOUBLE);
			eval.Return(result ? Atoms.TRUE : Atoms.FALSE);
		}
